...
class Mage_Core_Controller_Varien_Front extends Varien_Object
{
    ...
    public function init()
    {
		...
		Mage::Log('Checking global config for routers at '.self::XML_STORE_ROUTERS_PATH.' for routers');
        ...
        foreach ($routersInfo as $routerCode => $routerInfo) {
            ...
            if (isset($routerInfo['class'])) {
				...
				Mage::Log("Instantiating Router: ".$routerInfo['class']);            	
    ...
    public function dispatch()
    {
        ...
        while (!$request->isDispatched() && $i++<100) {
            foreach ($this->_routers as $router) {
            	Mage::Log("Matching router [".array_search($router,$this->_routers)."]" . 
            	" = ".get_class($router)." " .
            	" vs. request");
    ...
    public function rewrite()
    {
        ...
        foreach ($config->children() as $rewrite) {
			...
			Mage::Log("Rewriting: $from -> $to AGAINST ".$request->getPathInfo());    
