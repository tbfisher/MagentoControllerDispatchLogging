...
class Mage_Core_Controller_Varien_Router_Standard extends Mage_Core_Controller_Varien_Router_Abstract
{
    ...
    public function collectRoutes($configArea, $useRouterName)
    {
		...
		Mage::Log(get_class($this) . ' is collecting routes from the global config at '.$configArea.'/routers');        
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        if (!$module) {
            if (Mage::app()->getStore()->isAdmin()) {
        	    ...
        	    Mage::Log("Found no \$module, failing match");
                return false;
            }
        }

        Mage::Log('Module: '.$module);
        /**
        ...
        if ($modules === false) {
            if ($moduleFrontName = $this->getModuleByName($module, $this->_modules)) {
				...
				Mage::Log("Found no \$moduleFrontName, failing match");            
                return false;
            }
        }
		Mage::Log('Setting Route Name ['.$this->getRouteByFrontName($module).'] on request');
        ...
        foreach ($modules as $realModule) {
            ...
            Mage::Log("Passing request into controller $controllerClassName");
            $controllerInstance = Mage::getControllerInstance($controllerClassName, $request, $front->getResponse());

            if (!$controllerInstance->hasAction($action)) {
				Mage::Log('MATCH FAIL: Could not find '.$action.' action on '.get_class($controllerInstance));            
                continue;
            }
		...
		Mage::Log("Setting Module Name to: ".$module.' at '.__METHOD__.'('.__LINE__.')');        
        $request->setModuleName($module);
		Mage::Log("Setting Controller Name to: ".$controller.' at '.__METHOD__.'('.__LINE__.')');                
        $request->setControllerName($controller);
        Mage::Log("Setting Action Name to: ".$action.' at '.__METHOD__.'('.__LINE__.')');                
		...
		Mage::Log("Calling dispatch method on ".get_class($controllerInstance));        
    ...
    protected function _validateControllerClassName($realModule, $controller)
    {
        ...
        if (!$this->validateControllerFileName($controllerFileName)) {
			Mage::Log("Found no valid controller file name for moule [$realModule] and controller [$controller], failing match (file name[$controllerFileName])");        
            return false;
        }
